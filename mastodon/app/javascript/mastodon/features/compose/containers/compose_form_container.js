...
import {
  pasteLinkCompose,
  appendTimeGridHashtagsFromUrl,
  pasteTimeGridLinkCompose,
  ...
  onPaste (e) {
    if (e.clipboardData && e.clipboardData.files.length === 1) {
      ...
      try {
        ...
        if (url.hostname === 'calendar.time-grid.org') {
          e.preventDefault();
          dispatch(pasteTimeGridLinkCompose({
            text: e.target.value,
            url: url.toString(),
        ...
        dispatch(appendTimeGridHashtagsFromUrl({ url: url.toString() }));
      } catch {
        return;
      }
    }

...
const MEDIA_CACHE_NAME = `${CACHE_NAME_PREFIX}media-v2`;
...
self.addEventListener('activate', function(event) {
  event.waitUntil((async () => {
    ...
    await Promise.all(keys.filter((key) => key === `m${CACHE_NAME_PREFIX}media` || key === `${CACHE_NAME_PREFIX}media`).map((key) => caches.delete(key)));
